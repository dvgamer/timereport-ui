...
export default {
  ...
  mounted() {
    this.$nextTick(function() {
      ...
      container.addEventListener(TOUCH_START, this.onTouchStart)

      container.addEventListener(TOUCH_END, this.onTouchEnd)
  ...
  methods: {
    ...
    onMouseDown({ target: bar, pageX: initialPageX, pageY: initialPageY }) {
      ...
      if (!className || !className.match || !className.match('scrolly-bar')) return
